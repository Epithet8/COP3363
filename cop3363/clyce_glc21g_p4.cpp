...
SUMMARY: this program will prompt the user for a file and then continue to ask the user if the file cant be open.
we then open and read this value and creat a string for data which will allow us to manipulate the math we need.
we will be putting out main function together by a series of functions we have been given and a few we have created. 
we will display judgescores and the average them as we omit the highest and lowest values. phone calls per reigion will be 
displayed and added together. if we have two contestants that have a tie we will create a pseudo random number 
to solve the tie.
 

OUTPUT: the output of this program would be as follows. First we will print our opening statments and intro
we will then prompt the user for the file information. We will then print the judge scores and the average of them
followed by the phone calls per reigon and finally we will add the phone values and display them
if we are given a tie our coinfilp will declare a winner. The final output will display our winners for judgescores
phonecalls and the overall winner followed by closing statments.


ASSUMPTIONS: we assume all data will be entered in a specific format the rest is stated above. we will also assume we cannout create any array or data structure
...
//then it wil return the data as a number
/*==========================================================================*/
int stoi(string num) 
{ 
    int number = 0; 
...
//allowing us to correctly us our math
